...
def _preprocessor(in_image, width, height, norm_flag=Normalization.positive, hist_eq=False):
...
def image_loader(base_dir, image_list, width, height, norm_flag=Normalization.positive, hist_eq=False):
    for img_path in image_list:
        img = io.imread(join(base_dir, img_path))
        img = _extra_preprocessor(img)
        yield _preprocessor(img, width, height, norm_flag, hist_eq)
